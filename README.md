...
- **Ver.4.5.0-beta**（2026-09-09）
  ...
  - 💬 AI 助手：引导性回答中的 Markdown 标题字号提升至约 20px（比正文大 2px）。
  - 📝 报案填报：被骗金额栏新增提示语与右侧「前往账单导入」按钮，可一键跳转账单导入页（中英俄三语）。
  - 🐛 修复：首页聊天发送后输入框未及时清空；识别页命中单一类型时「全类型诈骗」标签被替换为具体类型名。
  - 🧱 `dist/` 已与根目录同步（补齐 i18n.js / anti-fraud-knowledge.js / 字体 / assets）。

- **Ver.4.4.0-beta**（2026-09-08）
  - 💬 AI 智能助手：每次回答强制引导用户走「情绪安抚 → 诈骗识别 → 一键填报 → 控告书」完整报案流程——强化 system 提示词 + 服务端注入 + 回复末尾固定附加引导；DeepSeek 深度识别同步注入。
...
- **Ver.4.5.0-beta** (2026-09-09)
  ...
  - 💬 AI assistant: Markdown headings in guidance replies raised to ~20px (2px larger than body text).
  - 📝 Report filing: the "Total Amount Lost" field now shows a hint plus a right-side "Go to Bill Import" button that jumps to the bill import page (zh/en/ru).
  - 🐛 Fixes: the home chat input wasn't cleared promptly after sending; the "All Fraud Types" label was replaced by the specific type name when a single category matched.
  - 🧱 `dist/` is now synced with the root (i18n.js / anti-fraud-knowledge.js / fonts / assets added).

- **Ver.4.4.0-beta** (2026-09-08)
  - 💬 AI Assistant: every reply now reliably guides the user through the full "emotional support → fraud detection → one-click report → criminal complaint" flow via a strengthened system prompt, server-side injection, and a fixed footer appended to each reply; DeepSeek deep analysis injects the same guidance.
...
- **Ver.4.5.0-beta** (2026-09-09)
  ...
  - 💬 ИИ-ассистент: заголовки Markdown в направляющих ответах увеличены примерно до 20px (на 2px больше основного текста).
  - 📝 Подача заявления: у поля «Общая сумма ущерба» появилась подсказка и кнопка «Перейти к импорту выписки» справа для перехода на страницу импорта (zh/en/ru).
  - 🐛 Исправления: поле ввода чата на главной не очищалось сразу после отправки; при совпадении одного типа метка «Мошенничество всех видов» заменялась названием конкретного типа.
  - 🧱 Каталог `dist/` синхронизирован с корнем (добавлены i18n.js / anti-fraud-knowledge.js / шрифты / assets).

- **Ver.4.4.0-beta** (2026-09-08)
  - 💬 ИИ-ассистент: каждый ответ теперь надёжно направляет пользователя по полному процессу «эмоциональная поддержка → распознавание мошенничества → заполнение в один клик → заявление о преступлении» через усиленный системный промпт, серверную инъекцию и фиксированную подсказку в конце каждого ответа; глубокая оценка DeepSeek внедряет то же руководство.
