...
function generateReport() {
    ...
    let formData = {
        ...
        accusedPhone: document.getElementById('accusedPhone').value.trim(),
        accusedWechat: document.getElementById('accusedWechat').value.trim(),
        accusedAlipay: document.getElementById('accusedAlipay').value.trim(),
        accusedBankCard: document.getElementById('accusedBankCard').value.trim(),
        accusedAddress: document.getElementById('accusedAddress').value.trim(),
...
// 归一化空值：空字符串/"未填写"/"未知" 统一视为空（兼容旧历史数据占位符）
function normalizeEmpty(v) {
    if (v === null || v === undefined) return '';
    let s = String(v).trim();
    if (!s) return '';
    const placeholders = [t('report.unfilled'), t('report.unknown'), '未填写', '未知'];
    if (placeholders.includes(s)) return '';
    return s;
}

// 渲染报案记录为表单式只读字段（与报案填报页 UI 一致）
function renderReportDetail(data) {
    const fields = [
        { label: t('report.name'), value: data.name },
        { label: t('report.idNo'), value: data.idNo },
        { label: t('report.phone'), value: data.phone },
        { label: t('report.address'), value: data.address },
        { label: t('report.accusedName'), value: data.accusedName },
        { label: t('report.accusedPhone'), value: data.accusedPhone },
        { label: t('report.accusedWechat'), value: data.accusedWechat },
        { label: t('report.accusedAlipay'), value: data.accusedAlipay },
        { label: t('report.accusedBankCard'), value: data.accusedBankCard },
        { label: t('report.accusedAddress'), value: data.accusedAddress },
        { label: t('report.fraudTime'), value: data.fraudTime },
        { label: t('report.fraudLocation'), value: data.fraudLocation },
        { label: t('report.contactMethod'), value: data.contactMethod },
        { label: t('report.fraudPlatform'), value: data.fraudPlatform },
        { label: t('report.fraudType'), value: data.fraudType },
        { label: t('report.fraudMoney'), value: data.fraudMoney },
        { label: t('report.fraudDetail'), value: data.fraudDetail },
    ];
    function fieldHTML(f) {
        const v = normalizeEmpty(f.value);
        return '<div class="form-group"><label>' + escapeHtml(f.label) + '</label><div class="detail-value">' + escapeHtml(v) + '</div></div>';
    }

    let html = '<div class="section-title">' + escapeHtml(t('report.section1')) + '</div>' + fields.slice(0, 4).map(fieldHTML).join('');
    html += '<div class="section-title">' + escapeHtml(t('report.section2')) + '</div>' + fields.slice(4, 10).map(fieldHTML).join('');
    html += '<div class="section-title">' + escapeHtml(t('report.section3')) + '</div>' + fields.slice(10).map(fieldHTML).join('');

    const evidenceParts = [];
    if (data.evidenceCount) evidenceParts.push(t('report.evidenceImageCount', { count: data.evidenceCount }));
    if (data.ocrText) evidenceParts.push(t('evidence.ocrLabel') + '\n' + data.ocrText);
    if (data.userInputText) evidenceParts.push(t('evidence.userLabel') + '\n' + data.userInputText);
    if (evidenceParts.length) {
        html += '<div class="section-title">' + escapeHtml(t('report.section4')) + '</div><div class="form-group"><div class="detail-value">' + escapeHtml(evidenceParts.join('\n\n')) + '</div></div>';
    }
    return html;
}
...
window.viewHistoryDetail = function(id) {
    ...
    let data = record.data;
    let bodyHTML = '';
    if (record.type === 'report' && data && typeof data === 'object') {
        bodyHTML = renderReportDetail(data);
    } else if (record.type === 'deepDetect') {
        bodyHTML = '<div class="form-group"><label>' + escapeHtml(t('history.type.deepDetect')) + '</label><div class="detail-value">' + renderMarkdown((data && data.reply) || '') + '</div></div>';
    } else if (record.type === 'detect') {
        bodyHTML = '<div class="form-group"><label>' + escapeHtml(t('history.type.detect')) + '</label><div class="detail-value">' + escapeHtml((data && data.result) || '') + '</div></div>';
    } else {
        bodyHTML = '<div class="form-group"><div class="detail-value">' + escapeHtml(typeof data === 'string' ? data : JSON.stringify(data)) + '</div></div>';
    }
    ...
    overlay.innerHTML = `
        <div style="background:var(--bg-surface);border-radius:16px;padding:24px;max-width:600px;max-height:70vh;overflow-y:auto;width:90%;">
            <h3 style="font-size:18px;font-weight:700;margin-bottom:12px;color:var(--text-primary);">${t('history.detailTitle')}</h3>
            <p style="font-size:13px;color:var(--text-tertiary);margin-bottom:12px;">${record.time} | ${historyTypeLabel(record)}</p>
            ${bodyHTML}
            <button style="margin-top:16px;padding:8px 20px;background:var(--primary);color:white;border:none;border-radius:8px;cursor:pointer;font-size:15px;" onclick="this.closest('div[style*=fixed]').remove()">${t('common.close')}</button>
        </div>
    `;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('gotoBillFromMoneyBtn')?.addEventListener('click', () => switchPage('billPage'));
