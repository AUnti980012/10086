...
(function () {
    ...
    const PAIRS = [
        ...
        ['ai.caseAnalysis', '【案情分析】', '[Case Analysis]'],
        ['ai.caseAnalysisDisclaimer', '使用Deepseek进行辅助案情分析，不作为最终解释', 'This is an auxiliary case analysis generated with DeepSeek and does not serve as a final interpretation.'],
        ['ai.fraudCategories', '识别诈骗类型时，必须且只能从以下八大类中选用其一：仿冒身份欺诈、购物类诈骗、活动类诈骗、利诱类欺诈、虚构险情欺诈、日常生活消费欺诈、钓鱼木马病毒类欺诈、其他新型违法类欺诈。', 'When identifying the fraud type, you MUST choose one from the following eight categories: Impersonation Fraud, Shopping Fraud, Activity Fraud, Lure/Inducement Fraud, Fabricated Danger Fraud, Daily-Life Consumption Fraud, Phishing & Trojan Fraud, Other New-Type Illegal Fraud.'],
        ['ai.caseAnalysisInstruction', '请按以下结构输出：先给出诈骗判定结论（是否疑似诈骗、疑似类型、风险等级），疑似类型必须使用规定的八大类命名；然后另起一行输出标题「【案情分析】」，其后用客观陈述句总结能从用户文本中提取的案情要素（时间、地点、金额、诈骗手段、大致经过），只写能确定的信息，不确定的不要编造。', 'Output as follows: first give a fraud judgment conclusion (whether fraud is suspected, the suspected type, and the risk level), and the suspected type MUST use one of the eight category names specified; then on a new line output the heading "[Case Analysis]", followed by an objective, declarative summary of the case facts that can be extracted from the user\'s text (time, place, amount, fraud method, approximate sequence) — state only what can be determined, do not fabricate.'],
        ['ai.flowGuide', '【强制要求：无论用户提问内容如何，每次回答都必须执行以下引导流程】你是反诈智能助手，请在每次回答中温和地引导用户：\n1. 情绪价值：先表达理解与共情，安抚用户情绪，肯定其及时求助；\n2. 引导识别：引导用户把聊天记录、短信等文本粘贴，或将截图上传到本系统「诈骗识别」页面，点击「开始识别」或「DeepSeek 深度判定」进行识别；\n3. 若识别为诈骗：引导用户点击「诈骗识别」页面下方的「一键填到报案表」按钮，把识别结果填入报案表；\n4. 说明原因：向用户说明填写《刑事控告书》能帮助公安机关相关人员快速理解案情、快速立案，材料越完整越有利；\n5. 引导填写：引导用户按「报案填报」页三步向导如实填写控告人信息、被控告人信息、被骗事实与证据。\n即使问题与诈骗无关，也要在回答末尾附上上述引导。全程语气温和、专业，不恐吓、不施压。', '【Mandatory: no matter what the user asks, every reply MUST follow this guidance flow】You are an anti-fraud assistant. In every reply, gently guide the user:\n1. Emotional support: first express understanding and empathy, soothe the user, and affirm their timely decision to seek help;\n2. Guide to detection: guide the user to paste chat records or SMS text, or upload screenshots to the "Fraud Detection" page, then click "Start Detection" or "DeepSeek Deep Analysis";\n3. If fraud is detected: guide the user to click the "Fill into Report" button at the bottom of the Fraud Detection page to fill the result into the report form;\n4. Explain why: explain that completing the Criminal Complaint helps law-enforcement personnel quickly understand the case and file it promptly — the more complete the materials, the better;\n5. Guide the form: guide the user to truthfully fill in the complainant information, accused information, and facts & evidence via the three-step wizard on the Report Filing page.\nEven if the question is unrelated to fraud, append this guidance at the end of your reply. Always keep a warm, professional tone; do not frighten or pressure.'],
        ['ai.guideFooter', '💡 如疑似遭遇诈骗：请到「诈骗识别」页粘贴聊天记录或上传截图进行识别；若确属诈骗，点击「一键填到报案表」生成《刑事控告书》，帮助警方快速理解案情、快速立案。', '💡 Suspect fraud? Go to the Fraud Detection page to paste chat records or upload screenshots for analysis; if confirmed, click "Fill into Report" to generate a Criminal Complaint, helping the police quickly understand the case and file it promptly.'],

        // ---- 诈骗识别页 ----
        ['identify.title', '诈骗识别', 'Fraud Detection'],
        ['identify.allTypes', '全类型诈骗', 'All Fraud Types'],
        ['identify.cat.impersonate', '仿冒身份欺诈', 'Impersonation Fraud'],
        ['identify.cat.shopping', '购物类诈骗', 'Shopping Fraud'],
        ['identify.cat.activity', '活动类诈骗', 'Activity Fraud'],
        ['identify.cat.lure', '利诱类欺诈', 'Lure / Inducement Fraud'],
        ['identify.cat.fiction', '虚构险情欺诈', 'Fabricated Danger Fraud'],
        ['identify.cat.consumption', '日常生活消费欺诈', 'Daily-Life Consumption Fraud'],
        ['identify.cat.phishing', '钓鱼、木马病毒类欺诈', 'Phishing & Trojan Fraud'],
        ['identify.cat.other', '其他新型违法类欺诈', 'Other New-Type Illegal Fraud'],
        ['load.keywords', '关键词字典', 'keyword dictionary'],
        ['load.ocr', 'OCR 引擎', 'OCR engine'],
        ['load.loading', '正在加载{name}…', 'Loading {name}…'],
        ['load.done', '加载完成：{name}', 'Loaded: {name}'],
        ['load.failed', '加载失败：{name}', 'Failed to load: {name}'],
        ...
        ['report.moneyImportHint', '若您有多笔账单，请点击右侧按钮前往「账单导入」页进行账单导入', 'If you have multiple bills, click the button on the right to go to the "Bill Import" page to import them.'],
        ['report.goToBillImport', '前往账单导入', 'Go to Bill Import'],
        ['report.evidenceImageCount', '证据图片：{count} 张', 'Evidence images: {count}'],
    ...
    const ru = {
        ...
        'ai.caseAnalysis': '[Анализ дела]',
        'ai.caseAnalysisDisclaimer': 'Данный вспомогательный анализ дела выполнен с помощью DeepSeek и не является окончательной трактовкой.',
        'ai.fraudCategories': 'При определении типа мошенничества используйте только одну из следующих восьми категорий: мошенничество с выдачей себя за других, мошенничество при покупках, мошенничество с акциями, мошенничество с заманчивыми предложениями, мошенничество с вымышленной опасностью, мошенничество в повседневных тратах, фишинговое/троянское мошенничество, прочие новые виды мошенничества.',
        'ai.caseAnalysisInstruction': 'Выводите ответ по следующей структуре: сначала дайте заключение о мошенничестве (есть ли подозрение, какой тип, уровень риска), при этом тип мошенничества должен использовать одно из указанных восьми наименований; затем с новой строки выведите заголовок «[Анализ дела]», после чего объективными повествовательными предложениями обобщите факты дела, которые можно извлечь из текста пользователя (время, место, сумма, способ мошенничества, примерная последовательность) — пишите только то, что можно установить, ничего не выдумывайте.',
        'ai.flowGuide': '【Обязательно: независимо от содержания вопроса, каждый ответ ДОЛЖЕН следовать приведённому ниже процессу руководства】Вы — антимошеннический ассистент. В каждом ответе мягко направляйте пользователя:\n1. Эмоциональная поддержка: сначала выразите понимание и сочувствие, успокойте пользователя и похвалите его за своевременное обращение за помощью;\n2. Направьте к распознаванию: предложите вставить текст переписки или SMS, либо загрузить скриншоты на страницу «Распознавание мошенничества» и нажать «Начать распознавание» или «Глубокая оценка DeepSeek»;\n3. Если выявлено мошенничество: предложите нажать кнопку «Заполнить форму заявления одним кликом» внизу страницы распознавания, чтобы перенести результат в форму заявления;\n4. Объясните причину: объясните, что заполнение «Заявления о преступлении» помогает сотрудникам органов быстро понять дело и оперативно возбудить его — чем полнее материалы, тем лучше;\n5. Направьте к заполнению: предложите честно заполнить сведения о заявителе, об обвиняемом, а также факты и доказательства по трёхшаговому мастеру на странице подачи заявления.\nДаже если вопрос не связан с мошенничеством, добавьте это руководство в конце ответа. Сохраняйте тёплый, профессиональный тон; не пугайте и не давите.',
        'ai.guideFooter': '💡 Подозреваете мошенничество? Перейдите на страницу «Распознавание мошенничества», вставьте переписку или загрузите скриншоты для анализа; при подтверждении нажмите «Заполнить форму заявления одним кликом», чтобы составить заявление о преступлении и помочь полиции быстрее понять дело и возбудить его.',

        // ---- 诈骗识别页 ----
        'identify.title': 'Распознавание мошенничества',
        'identify.allTypes': 'Мошенничество всех видов',
        'identify.cat.impersonate': 'Мошенничество с выдачей себя за других',
        'identify.cat.shopping': 'Мошенничество при покупках',
        'identify.cat.activity': 'Мошенничество с акциями',
        'identify.cat.lure': 'Мошенничество с заманчивыми предложениями',
        'identify.cat.fiction': 'Мошенничество с вымышленной опасностью',
        'identify.cat.consumption': 'Мошенничество в повседневных тратах',
        'identify.cat.phishing': 'Фишинговое / троянское мошенничество',
        'identify.cat.other': 'Прочие новые виды мошенничества',
        'load.keywords': 'словарь ключевых слов',
        'load.ocr': 'движок OCR',
        'load.loading': 'Загрузка {name}…',
        'load.done': 'Загружено: {name}',
        'load.failed': 'Ошибка загрузки: {name}',
        ...
        'report.moneyImportHint': 'Если у вас несколько платежей, нажмите кнопку справа, чтобы перейти на страницу «Импорт выписки» и импортировать их.',
        'report.goToBillImport': 'Перейти к импорту выписки',
        'report.evidenceImageCount': 'Изображения-доказательства: {count}',
