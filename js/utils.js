...
function renderMarkdown(text) {
    ...
    // 3. 标题（支持 # 到 ######）
    html = html.replace(/^######\s+(.+)$/gm, '<h6 style="font-size:14px;color:var(--text-secondary);margin:8px 0 4px;">$1</h6>');
    html = html.replace(/^#####\s+(.+)$/gm, '<h5 style="font-size:15px;color:var(--text-secondary);margin:10px 0 4px;">$1</h5>');
    html = html.replace(/^####\s+(.+)$/gm, '<h4 style="font-size:17px;font-weight:700;color:var(--text-primary);margin:12px 0 6px;">$1</h4>');
    html = html.replace(/^###\s+(.+)$/gm, '<h3 style="font-size:18px;font-weight:700;color:var(--primary);margin:14px 0 6px;">$1</h3>');
    html = html.replace(/^##\s+(.+)$/gm, '<h2 style="font-size:20px;font-weight:700;color:var(--primary);margin:16px 0 8px;">$1</h2>');
